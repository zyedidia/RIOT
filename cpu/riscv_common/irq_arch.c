...
static void trap_entry(void)
{
    ...
    if (prev_active_thread) {
        // save all registers
        regs->sp -= sizeof(regs_t);
        volatile regs_t* prev_thread_regs = (volatile regs_t*) regs->sp;
        *prev_thread_regs = *regs;
        prev_thread_regs->pc = read_csr(mepc);
        prev_active_thread->sp = (char*) regs->sp;
    }

    volatile regs_t* thread_regs = (volatile regs_t*) (void*) sched_active_thread->sp;
    *regs = *thread_regs;
    write_csr(mepc, thread_regs->pc);
    regs->sp += sizeof(regs_t);
    __asm__ volatile ("mret");
}
